...
{
    int state = std::atomic_load_explicit(&mutex->state, std::memory_order_relaxed);
    if (state & MUTEX_LOCKED)
        return false;

    state = std::atomic_fetch_or_explicit(&mutex->state, MUTEX_LOCKED, std::memory_order_relaxed);
    if (state & MUTEX_LOCKED)
        return false;

    thread_fence(&mutex->state, std::memory_order_acquire);
    return true;
}
...
static inline void mutex_lock_default(mutex_t *mutex)
{
    ...
    int state = std::atomic_exchange_explicit(&mutex->state, MUTEX_LOCKED | MUTEX_SLEEPING, std::memory_order_relaxed);

    while (state & MUTEX_LOCKED) {
        futex_wait(&mutex->state, MUTEX_LOCKED | MUTEX_SLEEPING);
        state = std::atomic_exchange_explicit(&mutex->state, MUTEX_LOCKED | MUTEX_SLEEPING, std::memory_order_relaxed);
    }
...
{
    int state = std::atomic_exchange_explicit(&mutex->state, 0, std::memory_order_release);
...
#define cmpxchg(obj, expect, desired) \
    std::atomic_compare_exchange_strong_explicit(obj, expect, desired, std::memory_order_relaxed, std::memory_order_relaxed)
